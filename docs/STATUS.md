...
| URL 쿼리 복원과 로그인 후 쿼리 복귀를 검증하는 Playwright E2E는 아직 없음 | 낮음 | 진행 예정 |
...
| URL 쿼리 복원 흐름을 Playwright E2E로 검증하고 브라우저별 차이를 확인 | 2026-03-27 | 반나절 |

## 다음 계획
- [ ] mock 사용자 기반 인증을 실제 사용자 계정 체계로 확장할지 결정
- [ ] 검색/태그 필터/페이지네이션 Playwright E2E 추가
- [ ] URL 쿼리 복원과 로그인 후 복귀를 Playwright E2E로 추가
